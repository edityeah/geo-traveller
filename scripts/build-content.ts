...
async function buildPosts(warnings: string[]) {
  ...
  // Collect old-WP-URL → new-/posts/-URL pairs for 301 redirects.
  ...
  for (const page of pages) {
    ...
    // Migrated posts carry their original WordPress URL. The new slug is
    // truncated to 80 chars, so the paths differ — emit an explicit 301 so
    // Google's already-indexed old URLs resolve to the new page instead of 404.
    if (props.originalUrl) {
      try {
        const fromPath = new URL(props.originalUrl).pathname;
        const toPath = `/posts/${props.slug}/`;
        if (fromPath && fromPath !== toPath && fromPath !== '/') {
          redirects.push({ from: fromPath, to: toPath });
        }
      } catch { /* ignore malformed originalUrl */ }
    }
  ...
  // Write Cloudflare Pages _redirects so old WordPress URLs (already indexed by
  // Google) 301 to the new /posts/ URLs instead of 404-ing.
  const header =
    '# AUTO-GENERATED by build-content.ts — old WordPress URLs → new /posts/ URLs.\n' +
    '# Recovers Google\'s existing index after the migration. Do not edit by hand.\n';
  const lines = redirects
    .map((r) => `${r.from}  ${r.to}  301`)
    .sort();
  await writeFile(join(ROOT, 'public', '_redirects'), header + lines.join('\n') + '\n');
  console.log(`[build-content] wrote ${redirects.length} legacy 301 redirects to public/_redirects`);
}
